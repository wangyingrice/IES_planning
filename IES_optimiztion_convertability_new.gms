...
************************************************* 注入综合能源系统的最大电功率和气功率
       Pmax_grid      Maximum Electric Power of PCC /20000/
...
**************************************************CHP机组参数
       ...
       Pmax_chp       Maximum Active Power of CHP /10000/
       ...
       Cmax_chp       Maximum Cool Power of CHP /7000/
       Hmax_chp       Maximum Heat Power of CHP /8000/
...
*********************成本函数
        ...
        Cost_FC            ..FC=e=lamb_C*sum(T,RC_p*P_grid(T)+RC_g*G_gas(T)/R_gas);
        EinFunc            ..Ein=e=sum(T,P_grid(T)+G_gas(T));
        EoutFunc           ..Eout=e=sum(T,C_chp(T)+C_pc(T)+CR_cs(T)+H_chp(T)+H_gb(T)+H_ph(T)+HR_hs(T)+G_gas(T)-G_chp(T)-G_gt(T)-G_gb(T)+Pmax_load*p_rate(T));
*********************冷功率平衡
