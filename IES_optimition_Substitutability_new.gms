...
**************************************************CHP机组参数
       ...
       Pmax_chp       Maximum Active Power of CHP /5000/
       gama_chp       Heating Ratio of Waste Heat of CHP /0.6/
       eta_cq         Effiency of Absorption Chiller of CHP /1.2/
       eta_hq         Effiency of Waste Heat Boiler of CHP /0.9/
       Cmax_chp       Maximum Cool Power of CHP /3500/
       Hmax_chp       Maximum Heat Power of CHP /4000/
************************************************** 燃气机组、燃气锅炉、电热炉、电制冷机参数
       Pmax_gt        Maximum Active Power of Gas Turbine /5000/
       eta_gt         Energy Efficiency of Gas Turbine /0.4/

       Hmax_gb        Maximum Heat Power of Gas Boiler /2000/
       eta_gb         Energy Efficiency of Gas Boiler /0.9/

       Hmax_ph        Maximum Heat Power of Electric Heater /2000/
       eta_ph         Energy Efficiency of Electric Heater /0.95/

       Cmax_pc        Maximum Cold Power of Electric Cooler /3500/
...
**************************************************设备投资价格
       Minv_chp       Insvesment cost per unit capacity of CHP /4500/
       Minv_rhe       Insvesment cost per unit capacity of Residual Heat Exchanger /340/
       Minv_ac        Insvesment cost per unit capacity of Absorption Chiller /1353/
...
**************************************************碳排放相关参数
       lamb_C         Carbon tax price /0.14/
       RC_p           CO2 emission coefficient of electricity /0.25/
       RC_g           CO2 emission coefficient of natural gas /1.85/
...
**************************************************分时电价和分时气价
       ...
       lamb_hot(T)    Buying Hot Price            /t1 0.225, t2 0.225, t3 0.225, t4 0.225, t5 0.225, t6 0.225, t7 0.225,
                                                   t8 0.225, t9 0.225, t10 0.225, t11 0.225, t12 0.225,
                                                   t13 0.225, t14 0.225, t15 0.225, t16 0.225,
                                                   t17 0.225, t18 0.225, t19 0.225, t20 0.225,
                                                   t21 0.225,t22 0.225, t23 0.225, t24 0.225/


       lamb_cold(T)   Buying Cold Price           /t1 0.65, t2 0.65, t3 0.65, t4 0.65, t5 0.65, t6 0.65, t7 0.65,
                                                   t8 0.65, t9 0.65, t10 0.65, t11 0.65, t12 0.65,
                                                   t13 0.65, t14 0.65, t15 0.65, t16 0.65,
                                                   t17 0.65, t18 0.65, t19 0.65, t20 0.65,
                                                   t21 0.65,t22 0.65, t23 0.65, t24 0.65/
**************************************************不可替代负荷数据
       Cmax_load      Rated Cold Load  /7000/
...
*************************************************** 替代型负荷
*********************电-冷替代型负荷参数
       Ppc_load_max      Maximun Electric Power of Electricity-Cool Alternative Load /825/
       Cpc_load_max      Maximun Cool Power of Electricity-Cool Alternative Load /825/
********************
       Lpc_load_max      Maximun Power of Electricity-Cool Alternative Load /3000/
       wpc_load          Efficiency Coefficient of Electric Load  /3.0/
       vpc_load          Efficiency Coefficient of Cool Load  /1.0/
*********************电-热替代型负荷参数
       Pph_load_max      Maximun Electric Power of Electricity-Heat Alternative Load /1546.875/
       Hph_load_max      Maximun Heat Power of Electricity-Heat Alternative Load /412.5/
********************
       Lph_load_max      Maximun Power of Electricity-Cool Alternative Load /1500/
       wph_load          Efficiency Coefficient of Electric Load  /0.8/
       vph_load          Efficiency Coefficient of Heat Load  /1.0/
***************************************************************************************
********************气-冷替代型负荷参数
       Ggc_load_max      Maximun Gas Power of Gas-Cool Alternative Load /2062.5/
       Cgc_load_max      Maximun Cool Power of Gas-Cool Alternative Load /825/
********************
       Lgc_load_max      Maximun Power of Gas-Cool Alternative Load /3000/
       wgc_load          Efficiency Coefficient of Gas Load  /1.2/
       vgc_load          Efficiency Coefficient of Cool Load  /1.0/
*********************气-热替代型负荷参数
       Ggh_load_max      Maximun Gas Power of Gas-Heat Alternative Load /2475/
       Hgh_load_max      Maximun Heat Power of Gas-Heat Alternative Load /412.5/
*********************
       Lgh_load_max      Maximun Power of Gas-Heat Alternative Load /1500/
       wgh_load          Efficiency Coefficient of Gas Load  /0.5/
       vgh_load          Efficiency Coefficient of Heat Load  /1.0/
;
...
positive Variables
      ...
      FH                 Cost of Buying Heat
      FC                 Cost of Buying Cold
...
*************************************可替代负荷
*********************电冷替代型负荷
      Ppc_load(T)           Electric Power of Alternative Load
      Cpc_load(T)           Cold Power of Alternative Load
*********************电热替代型负荷
      Pph_load(T)           Electric Power of Alternative Load
      Hph_load(T)           Heat Power of Alternative Load
*********************气-冷替代型负荷
      Ggc_load(T)           Gas Power of Alternative Load
      Cgc_load(T)           Cool Power of Alternative Load
*********************气热替代型负荷
      Ggh_load(T)           Gas Power of Alternative Load
      Hgh_load(T)           Heat Power of Alternative Load
...
*********************电-冷替代型负荷功率供给限制
       Ppc_load.lo(T)=0;
       Ppc_load.up(T)=Ppc_load_max;
       Cpc_load.lo(T)=0;
       Cpc_load.up(T)=Cpc_load_max;
*********************电-热替代型负荷功率供给限制
       Pph_load.lo(T)=0;
       Pph_load.up(T)=Pph_load_max;
       Hph_load.lo(T)=0;
       Hph_load.up(T)=Hph_load_max;
*********************气-冷替代型负荷功率供给限制
       Ggc_load.lo(T)=0;
       Ggc_load.up(T)=Ggc_load_max;
       Cgc_load.lo(T)=0;
       Cgc_load.up(T)=Cgc_load_max;
*********************气-热替代型负荷功率供给限制
       Ggh_load.lo(T)=0;
       Ggh_load.up(T)=Ggh_load_max;
       Hgh_load.lo(T)=0;
       Hgh_load.up(T)=Hgh_load_max;
***********************************************
Equations
      ObjFunc                     Total Cost of IES Equation
      Cost_FP                     Cost for Buying Electricity
      Cost_FG                     Cost for Buying Natural Gas
      Cost_FH                     Cost for Buying Heat
      Cost_FC                     Cost for Buying Cold
      Cost_FS                     Cost for ESU Degeneration
      Cost_FINV                   Cost of equipment investment
      Cost_FM                     Cost of equipment maintenance
      Cost_FCO2                    Cost of CO2 emissions
      EoutFunc
      EinFunc
***************************************
...
***************************************
      PC_AL_Load(T)                  Electricity-Cool Alternative Load Equation
      PH_AL_Load(T)                  Electricity-Heat Alternative Load Equation
      GC_AL_Load(T)                  Electricity-Gas Alternative Load Equation
      GH_AL_Load(T)                  Gas-Heat Alternative Load Equation
...
;
*********************成本函数
        ObjFunc            ..Obj=e=FM+FP+FG+FS+FH+FC;
        Cost_FP            ..FP=e=sum(T,lamb_grid(T)*P_grid(T));
        Cost_FG            ..FG=e=sum(T,lamb_gas(T)*G_gas(T)/R_gas);
        Cost_FH            ..FH=e=sum(T,lamb_hot(T)*(Hph_load(T)+Hgh_load(T)));
        Cost_FC            ..FC=e=sum(T,lamb_cold(T)*(Cpc_load(T)+Cgc_load(T)));
        Cost_FS            ..FS=e=sum(T,lamb_ps*PS_ps(T)+lamb_hs*HS_hs(T)+lamb_cs*CS_cs(T));
        Cost_FINV          ..FINV=e=0.078*Minv_chp*Pmax_chp+0.103*Minv_rhe*Hmax_chp+0.103*Minv_ac*Cmax_chp+0.073*Minv_gt*Pmax_gt+0.087*Minv_gb*Hmax_gb+0.103*Minv_pc*Cmax_pc+0.087*Minv_ph*Hmax_ph+0.087*Minv_CSU*Smax_cS+0.087*Minv_HSU*Smax_hS+0.103*Minv_BSU*Smax_ps;
        Cost_FM            ..FM=e=sum(T,lamb_chp*P_chp(T)+lamb_rhe*H_chp(T)+lamb_ac*C_chp(T)+lamb_gt*P_gt(T)+lamb_gb*H_gb(T)+lamb_pc*C_pc(T)+lamb_ph*H_ph(T));
        Cost_FCO2          ..FCO2=e=lamb_C*sum(T,RC_p*P_grid(T)+RC_g*G_gas(T)/R_gas);
        EinFunc            ..Ein=e=sum(T,P_grid(T)+G_gas(T)+Hph_load(T)+Hgh_load(T)+Cpc_load(T)+Cgc_load(T));
        EoutFunc           ..Eout=e=sum(T,C_chp(T)+C_pc(T)+CR_cs(T)+H_chp(T)+H_gb(T)+H_ph(T)+HR_hs(T)+G_gas(T)-G_chp(T)-G_gt(T)-G_gb(T)+Pmax_load*p_rate(T)+Lpc_load_max*c_rate(T)+Lph_load_max*h_rate(T)+Lgc_load_max*p_rate(T)+Lgh_load_max*g_rate(T));
*********************冷功率平衡
        Balance_C(T)       ..C_chp(T)+C_pc(T)+CR_cs(T)-CS_cs(T)=e=Cmax_load*c_rate(T)+Cpc_load(T)+Cgc_load(T);
        Balance_H(T)       ..H_chp(T)+H_gb(T)+H_ph(T)+HR_hs(T)-HS_hs(T)=e=Hmax_load*h_rate(T)+Hph_load(T)+Hgh_load(T);
        Balance_P(T)       ..P_grid(T)+P_chp(T)+P_gt(T)+PR_ps(T)-PS_ps(T)-P_ph(T)-P_pc(T)=e=Pmax_load*p_rate(T)+Ppc_load(T)+Pph_load(T);
        Balance_G(T)       ..G_gas(T)-G_chp(T)-G_gt(T)-G_gb(T)=g=Gmax_load*g_rate(T)+Ggc_load(T)+Ggh_load(T);
*******************可替代负荷等式约束
        PC_AL_Load(T)         ..wpc_load*Ppc_load(T)+vpc_load*Cpc_load(T)=e=Lpc_load_max*c_rate(T);
        PH_AL_Load(T)         ..wph_load*Pph_load(T)+vph_load*Hph_load(T)=e=Lph_load_max*h_rate(T);
        GC_AL_Load(T)         ..wgc_load*Ggc_load(T)+vgc_load*Cgc_load(T)=e=Lgc_load_max*p_rate(T);
        GH_AL_Load(T)         ..wgh_load*Ggh_load(T)+vgh_load*Hgh_load(T)=e=Lgh_load_max*g_rate(T);
*********************CHP设备运行效率
